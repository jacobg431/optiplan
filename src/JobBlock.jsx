...
export default function JobBlock({ job, jobStyle, isExpanded, expandedJobId, toggleJob }) {
    ...
    return (
        <>
            ...
            {isExpanded &&
                (() => {
                    const leftPercent = parseFloat(jobStyle.left) // e.g. "85%" → 85
                    const boxWidth = 22.6 // estimate info box width in %, adjust if needed
                    const adjustedLeft = leftPercent + boxWidth > 100 ? `${100 - boxWidth}%` : `${leftPercent}%`
